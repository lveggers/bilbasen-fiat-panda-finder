...
class TestConditionParsing:
    ...
    def test_parse_condition_good_conditions(self):
        ...
        for condition in good_conditions:
            score, debug_info = parse_condition(condition)
            assert 0.7 <= score <= 0.9, f"Expected good score for '{condition}', got {score}"
    ...
    def test_parse_condition_average_conditions(self):
        ...
        for condition in average_conditions:
            score, debug_info = parse_condition(condition)
            assert 0.4 <= score <= 0.7, f"Expected average score for '{condition}', got {score}"
    ...
    def test_parse_condition_empty_and_none(self):
        ...
        for condition in test_cases:
            score, debug_info = parse_condition(condition)
            assert score == 0.5, f"Expected neutral score for empty condition, got {score}"
    ...
    def test_parse_condition_with_modifiers(self):
        ...
        for modified, base in positive_cases:
            ...
            assert modified_score >= base_score, f"'{modified}' should score higher than '{base}'"
    ...
    def test_parse_condition_case_insensitive(self):
        ...
        for upper, lower in test_cases:
            ...
            assert upper_score == lower_score, f"Case should not matter: {upper} vs {lower}"
    ...
    def test_parse_condition_danish_characters(self):
        ...
        for condition in danish_conditions:
            score, debug_info = parse_condition(condition)
            assert 0.0 <= score <= 1.0, f"Score should be valid for Danish text: {condition}"
...
class TestConditionUtilityFunctions:
    ...
    def test_normalize_text(self):
        ...
        for input_text, expected in test_cases:
            ...
            assert result == expected, f"Normalization failed: '{input_text}' -> '{result}' (expected '{expected}')"
    ...
    def test_calculate_base_score_known_conditions(self):
        ...
        expected_score = max(CONDITION_MAPPINGS[phrase] for phrase in phrases if phrase in CONDITION_MAPPINGS)
    ...
    def test_get_condition_description(self):
        ...
        for score, expected in test_cases:
            ...
            assert result == expected, f"Expected '{expected}' for score {score}, got '{result}'"
...
class TestConditionEdgeCases:
    ...
    def test_mixed_positive_negative_conditions(self):
        ...
        for condition in mixed_conditions:
            ...
            assert len(effects) > 0, f"Should have some modifier effects for: {condition}"
...
def test_condition_mappings_completeness():
    ...
    for condition, score in CONDITION_MAPPINGS.items():
        ...
        assert isinstance(condition, str), f"Condition key should be string: {condition}"
    ...
    for category, examples in expected_categories.items():
        for example in examples:
            assert any(example in condition for condition in CONDITION_MAPPINGS), \
