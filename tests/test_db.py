...
class TestDatabaseSetup:
    ...
    def test_create_db_and_tables(self, temp_db):
        ...
        with Session(temp_db) as session:
            result = session.exec("SELECT name FROM sqlite_master WHERE type='table'").fetchall()
...
class TestListingCRUD:
    ...
    def test_get_listing_by_url(self, test_session, sample_listings):
        ...
        retrieved_listing = ListingCRUD.get_listing_by_url(test_session, original_listing.url)
    ...
    def test_get_listing_by_url_not_found(self, test_session):
        ...
        result = ListingCRUD.get_listing_by_url(test_session, "https://nonexistent.com/listing")
    ...
    def test_get_listings_with_price_filter(self, test_session, sample_listings):
        ...
        expensive_listings = ListingCRUD.get_listings(
            test_session, 
            min_price=80000
        )
        
        assert all(listing.price_dkk >= 80000 for listing in expensive_listings if listing.price_dkk)
        assert len(expensive_listings) < len(sample_listings)
        
        # Filter for cheap listings (<= 60000)
        cheap_listings = ListingCRUD.get_listings(
            test_session,
            max_price=60000
        )
        
        assert all(listing.price_dkk <= 60000 for listing in cheap_listings if listing.price_dkk)
    ...
    def test_get_listings_with_year_filter(self, test_session, sample_listings):
        ...
        new_cars = ListingCRUD.get_listings(
            test_session,
            min_year=2020
        )
        ...
        old_cars = ListingCRUD.get_listings(
            test_session,
            max_year=2018
        )
    ...
    def test_get_listings_with_km_filter(self, test_session, sample_listings):
        ...
        low_km = ListingCRUD.get_listings(
            test_session,
            max_km=50000
        )
        
        assert all(listing.kilometers <= 50000 for listing in low_km if listing.kilometers)
        
        # Filter for high mileage (>= 100000)  
        high_km = ListingCRUD.get_listings(
            test_session,
            min_km=100000
        )
        
        assert all(listing.kilometers >= 100000 for listing in high_km if listing.kilometers)
