...
class BilbasenScraper:
    ...
    async def scrape_search_results(self, max_pages: int = 5) -> List[ScrapedListing]:
        ...
        """
        logger.info(f"Starting scrape of Bilbasen search results for '{settings.search_term}'")
        all_listings = []
            ...
            for page_num in range(1, max_pages + 1):
                try:
                    ...
                    try:
                        ...
                        if not listing_urls:
                            ...
                            if page_num == 1:
                                ...
                                no_results = await client.extract_text(
                                    page, 
                                    get_selector("search", "no_results")
                                )
                                if no_results:
                        ...
                        page_listings = await self._extract_search_page_listings(page, listing_urls)
    ...
    async def _extract_listing_urls(self, page) -> List[str]:
        ...
        try:
            ...
            listing_link_elements = await page.query_selector_all(get_selector("search", "listing_link"))
            links = []
    ...
    async def _extract_search_page_listings(self, page, urls: List[str]) -> List[ScrapedListing]:
        ...
        try:
            ...
            for i, container in enumerate(listing_containers):
                ...
                try:
                    ...
                    logger.warning(f"Failed to extract data from listing container {i}: {e}")
                    # Still add URL-only listing
                    if i < len(urls):
                        listing = ScrapedListing(
                            title=f"Fiat Panda Listing {i+1}",
                            url=urls[i]
                        )
                        listings.append(listing)
        ...
        async with get_playwright_client() as client:
            try:
                ...
                try:
                    ...
                    title = await client.extract_text(page, get_selector("detail", "title"))
                    if title:
                    ...
                    logger.info(f"Successfully scraped details for: {scraped_listing.url}")
    ...
    async def _extract_from_specs_table(self, page, scraped_listing: ScrapedListing):
        ...
        try:
            specs_table = await page.query_selector(get_selector("detail", "specs_table"))
            if not specs_table:
                return
            
            rows = await specs_table.query_selector_all(get_selector("detail", "specs_rows"))
            
            for row in rows:
                try:
                    label_element = await row.query_selector(get_selector("detail", "spec_label"))
                    value_element = await row.query_selector(get_selector("detail", "spec_value"))
                    ...
                    elif not scraped_listing.kilometers_text and ("km" in label or "kilometer" in label):
                        scraped_listing.kilometers_text = value
                    elif not scraped_listing.fuel_type_text and ("brændstof" in label or "fuel" in label):
                        scraped_listing.fuel_type_text = value
                    elif not scraped_listing.transmission_text and ("gear" in label or "transmission" in label):
                        scraped_listing.transmission_text = value
                    elif not scraped_listing.condition_text and ("stand" in label or "condition" in label):
