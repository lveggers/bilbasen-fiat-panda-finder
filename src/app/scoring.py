...
class ListingScorer:
    ...
    def score_listings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Scoring completed. Score range: {scored_df['score'].min():.1f} - {scored_df['score'].max():.1f}")
    ...
    def _score_price(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if min_price == max_price:
            ...
            normalized_prices = (winsorized_prices - min_price) / (max_price - min_price)
        ...
        logger.debug(f"Price scoring: {len(prices)} valid prices, range {min_price:.0f}-{max_price:.0f} DKK")
    ...
    def _score_year(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Year scoring: {len(years)} valid years, range {min_year}-{max_year}")
    ...
    def _score_kilometers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'kilometers' not in df.columns:
            logger.warning("No kilometers column found, setting kilometers_score to 0.5")
        ...
        if not km_mask.any():
            logger.warning("No valid kilometers found, setting all kilometers_score to 0.5")
            df['kilometers_score'] = 0.5
            return df
        ...
        logger.debug(f"Kilometers scoring: {len(kilometers)} valid km values, range {min_km:.0f}-{max_km:.0f}")
    ...
    def _score_condition(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Condition scoring: range {df['condition_score'].min():.2f}-{df['condition_score'].max():.2f}")
    ...
    def _calculate_final_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for component in ['price_score', 'year_score', 'kilometers_score', 'condition_score']:
