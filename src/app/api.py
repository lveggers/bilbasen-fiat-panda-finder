...
async def get_score_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve score statistics"
        )


@app.get("/scores/distribution", tags=["Scores"])
async def get_score_distribution(session: Session = Depends(get_session)) -> List[int]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve score distribution"
        )
...
async def get_detailed_score_breakdown(
    ...
    try:
        ...
        if not listings:
            return {"error": "No listings found", "total_listings": 0}
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate score breakdown"
        )
...
async def scrape_sync(
    ...
    try:
        ...
        logger.info(
            f"Synchronous scraping completed: {stored_count} stored, {scored_count} scored"
        )
...
async def rescore_listings(session: Session = Depends(get_session)) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"message": "Rescoring completed", "updated_count": updated_count}
...
async def scrape_and_store_listings(
    max_pages: int, include_details: bool, session: Session
):
